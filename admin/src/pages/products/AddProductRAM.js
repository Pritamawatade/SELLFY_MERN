  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:4000/api/productRAM/create', {
                method: 'POST',
                headers: { 
                            ...
                            >
                                <div className="basic-info space-y-6">
                                    ...
                                    <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-6" variants={itemVariants}>
                                        ...
                                        <div>
                                            <label className="text-gray-700 dark:text-gray-300 mb-2 font-semibold text-sm uppercase tracking-wider"> Proudct RAM</label>
                                            <input
                                                type="text"
                                                value={productRAM}
                ...
                >
                    <div className="overflow-x-auto">
                        <table className="min-w-full bg-white dark:bg-[#1E293B] rounded-lg shadow-lg">
                            <thead>
                                <tr>
                                    <th className="px-6 py-3 border-b-2 border-gray-300 dark:border-gray-700 text-left leading-4 text-blue-500 dark:text-blue-300 tracking-wider">Index</th>
                                    <th className="px-6 py-3 border-b-2 border-gray-300 dark:border-gray-700 text-left leading-4 text-blue-500 dark:text-blue-300 tracking-wider">Product RAM</th>
                                </tr>
                            </thead>
                            <tbody>
                                {  
                                productRAMs.length > 0 && productRAMs?.map((ram, index) => (
                                    <motion.tr
                                        key={index}
                                    ...
                                    >
                                        <td className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">{index + 1}</td>
                                        <td className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">{ram?.productRAM}</td>
                                    </motion.tr>
                                ))}
                            </tbody>
                        </table>
