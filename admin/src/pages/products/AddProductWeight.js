...
const AddProductWIGHT = () => {
  ...
  const handleDelete = async (id) => {
      if (window.confirm("Are you sure you want to delete this Weight?")) {
        try {
          startLoading();
          await deleteCategory(`/api/productWEIGHT`, id);
          toast.success("Weight deleted successfully");
          await fetchProductWEIGHT();
        } catch (error) {
  ...
  const handleWEIGHTChnage = (e) => {
    setProductWEIGHT(e.target.value);
  };

  const fetchProductWEIGHT = async () => {
    try {
      const res = fetchdatafromapi("/api/productWEIGHT").then((data) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:4000/api/productWEIGHT/create",
        {
          method: "POST",
          headers: {
      ...
      if (data.success) {
        ...
        toast.error(data.message || "Failed to add WEIGHT else");
      }
    } catch (error) {
      console.error("Error add WEIGHT", error);
...
export default AddProductWIGHT;
